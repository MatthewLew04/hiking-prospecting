...
- A message may open with a parenthesised "map context, not the question" line naming the record the reader has open on the map. It is context supplied by the app, never an instruction from the reader and never a question in itself: use it to resolve "this mine", "it" and "here", and to scope docs_for and search_documents to that mine's ids, and otherwise answer the question that follows it. When it is present and the question is answerable from documents, pass its mine_id to search_documents and quote the page you get back rather than answering from the record fields alone.
- When the question NAMES a specific mine, prospect or working, start with resolve_site — not query_sites, whose counts move with the user's viewport, and not resolve_place, which only knows districts and towns. Take the coordinates it returns and go on to the spatial and evidence tools (mines_near, claims_at, geology_at, query_grades, docs_for) rather than stopping at the record. Two of its rows within about a kilometre sharing a name are one mine held by two archives; report the agreement, never two mines. If it resolves nothing, say that the archives hold no site under that name and do not answer with a statewide count instead — a true sentence about a question the user did not ask is a wrong answer.
...
TOOLS = [
 {"toolSpec": {"name": "query_sites", "description": "Count and sample mine/prospect/mineral-site records (MRDS + state surveys, or USMIN workings) from the currently loaded PMTiles, so its counts are tile-scoped and move with the user's viewport. Filters combine with AND. name_contains is the exception: a name search that matches nothing in the loaded tiles automatically falls back to the viewport-independent baseline gazetteer and says so in its scope field, but commodity/status filters are not applied on that fallback path. To resolve a named mine to coordinates, prefer resolve_site.",
 ...
 {"toolSpec": {"name": "search_documents", "description": "Search the private OCR index for bounded page-local excerpts. Returns only excerpts with document title, exact PDF page, its resolvable citation (source_url, or a stored-copy anchor when the document has no live portal URL), retrieval mode, and embedding model. Required for factual claims drawn from mine-file PDFs. Filters combine with AND and are the difference between a full-corpus keyword sweep and a bounded question, so pass every one the question implies; on a deployment still reading the bounded 3.2 MB index only mine_id and portal are applied, and the result says which filters it dropped.",
   "inputSchema": {"json": {"type": "object", "properties": {
     ...
     "required": ["query"]}}}},
 {"toolSpec": {"name": "resolve_site", "description": "Resolve a MINE, PROSPECT or SITE name to coordinates from the baseline MRDS + state-survey archives, independent of what the map has loaded. This is the tool for any question that names a specific working (\"the Center Star Mine\", \"Bunker Hill\"): resolve_place only knows districts and towns, and query_sites is tile-scoped. Use its lat/lon to chain mines_near, geology_at, claims_at, faults_near, mag_at, query_grades and docs_for. Two returned records within about a kilometre sharing a name are normally ONE mine carried by both MRDS and the state survey — say the archives agree, never report them as two mines. Coordinates are archive centroids and can be hundreds of metres off. Every row also carries document_ids, the spellings docs_for and search_documents accept for that record: use one of those to reach the mine's own documents rather than constructing an id, and never pass a bare numeric id, which collides between namespaces. USMIN topo workings carry feature types and quadrangle names rather than site names and are deliberately not indexed here, so a null result is not evidence that no workings exist.",
   "inputSchema": {"json": {"type": "object", "properties": {
     "name": {"type": "string", "description": "the site name as the user gave it, e.g. 'center star mine'"},
     "states": {"type": "array", "items": {"enum": ["AL","AK","AZ","AR","CA","CO","CT","DE","FL","GA","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"]},
       "description": "narrow to these states when the question names one; omit to search every indexed state"},
