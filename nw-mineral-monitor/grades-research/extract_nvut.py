...
import csv, json, collections, shapefile, re, glob, os
...
for code, b in by.items():
    ...
    open(OUT+f'sites/mrds_{code.lower()}.json','w').write(json.dumps(o, separators=(',',':'), ensure_ascii=False))
...
for code, b in ub.items():
    ...
    open(OUT+f'sites/usmin_{code.lower()}.json','w').write(json.dumps(o, separators=(',',':'), ensure_ascii=False))
...
sts = json.load(open(OUT+'boundaries/states.json'))
...
open(OUT+'boundaries/states.json','w').write(json.dumps(sts, separators=(',',':')))
cos = json.load(open(OUT+'boundaries/counties.json'))
FIPS = {'32':'NV','49':'UT'}
...
man['districts']['auto']['n'] = len(allitems)
open(OUT+'manifest.json','w').write(json.dumps(man, separators=(',',':')))
print('manifest totals:', man['totals'])
