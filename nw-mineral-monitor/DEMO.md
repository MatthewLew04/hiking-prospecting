...
1. Header **WATCH** button → the digest panel (generated by the daily
   Lambda; also emailed if SES is configured). Each alert: serial,
...
3. Open `…/data/alerts/latest.json` — same digest as JSON for the webhook.
